...
def process_accommodation_record(record: dict, schema: dict) -> dict:
    # Crear DF inferido
    accommodation_schema_dict = json.loads(open('/opt/airflow/schema/accommodation_schema.json').read())
...
def process_weather_record(raw: dict, schema: dict) -> dict:
    weather_schema_dict = json.loads(open('/opt/airflow/schema/weather_schema.json').read())
    ...
    if not inspector.has_table(table_name):
        ...
        try:
            ...
            df.write \
                ...
                .option("password", "your_pwrd") \
                .option("driver", "your_driver") \
                .mode("overwrite") \
                .save()
    ...
    for single_date in tqdm([start + i * delta for i in range((end - start).days + 1)]):
        ...
        for city, coord in coords.items():
            ...
            if file_exists(fs, trusted_path):
                ...
                weather_schema_dict = json.loads(open('/opt/airflow/schemas/weather_schema.json').read())
                weather_struct = build_struct(weather_schema_dict)
                df_valid = spark.createDataFrame(data_rows, schema=weather_struct)
...
if __name__ == '__main__':
    ...
    file_system_client = datalake_client.get_file_system_client('name_conteiner')
    conn_str = "postgresql://your_user:your_pwd@postgres-db:5432/postgres"
    ...
    # sync on the fly
    get_and_sync_accommodation(file_system_client, spark, start, end, destination_ids, accommodation_query, headers, '/opt/airflow/schemas/accommodation_schema.json')
    backfill_trusted_zone(file_system_client, 'accommodation/', '/opt/airflow/schemas/accommodation_schema.json')
